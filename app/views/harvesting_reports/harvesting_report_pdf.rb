...
class HarvestingReportPdf < Prawn::Document
  ...
  def draw_header
    repeat :all do
      ...
      font "Courier" do
        text_box "Hou D.O.B  Wks Die | 1day 2day 3day 4day  Avg | 1day 2day 3day 4day  Avg | Collector",
    ...
    bounding_box [15.mm, bounds.top - 12.mm], width: 250.mm do
      ...
      text(
        @rows.map do |r|
          ...
          avg_yield = (yield_1 + yield_2 + yield_3 + yield_4)/4
          avg_prod  = (r['production_1'].to_f + r['production_2'].to_f + r['production_3'].to_f + r['production_4'].to_f)/4
          start_mark_for_warning(r['production_1'].to_f, avg_prod) +
          "#{r['house_no']} #{r['dob'].to_date.strftime('%y%m%d')}  #{("%2d") % r['age'].to_i.to_s} " +
          "#{("%3d") % r['death'].to_f} | " +
          "#{("%4d") % r['production_1'].to_f} " +
          "#{("%4d") % r['production_2'].to_f} " +
          "#{("%4d") % r['production_3'].to_f} " +
          "#{("%4d") % r['production_4'].to_f} " +
          "<b>#{("%4d") % avg_prod}</b> |" +
          "#{("%4d%") % yield_1}" +
          "#{("%4d%") % yield_2}" +
          "#{("%4d%") % yield_3}" +
          ...
          end_mark_for_warning(r['production_1'].to_f, avg_prod)
        end.join("\n"), inline_format: true)
      text("=========================================================================================", style: :bold)
      text(
