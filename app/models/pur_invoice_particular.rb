...
private
  ...
  def particular_type_transaction
    Transaction.new({ 
      doc: doc, 
      account: particular_type.account || Account.find_by_name1('General Purchases'), 
      transaction_date: doc.doc_date, 
      note: doc.supplier.name1 + " - " + particular_type.name,
  ...
  def gst_transaction
    if gst != 0
      Transaction.new({
        ...
        transaction_date: doc.doc_date, 
        note: doc.supplier.name1 + ' - GST on ' + particular_type.name,
