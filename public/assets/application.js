...
(function() {
  window.advance = {
    init: function() {
      ...
      return ($('form')).on('blur', '#advance_pay_from_name1', function() {
        if (($(this)).val() === 'Cash In Hand') {
          ($('#advance_chq_no')).parents('.control-group').hide();
          return ($('#advance_chq_no')).val('');
        } else {
          ($('#advance_chq_no')).parents('.control-group').show();
          return ($('#advance_chq_no')).select();
        }
      });
    }
  };
...
(function() {
  window.asset = {
    init: function() {
      ...
      return app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide');
    },
    addition_init: function() {
...
(function() {
  window.cash_sale = {
    init: function() {
      invoice.shared_init();
      cheque.init();
      app.typeahead_init('.tax_code', '/cash_sale/typeahead_code');
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.tax_code'), '/cash_sale/typeahead_code');
      });
...
(function() {
  window.chart_of_account = {
    init: function() {
      ($('#css-treeview-expand')).click(function() {
        return $('.css-treeview input[type=checkbox]').attr('checked', true);
      });
      ($('#css-treeview-collapse')).click(function() {
        return $('.css-treeview input[type=checkbox]').attr('checked', false);
      });
      return ($('#css-treeview-expand')).click();
    }
  };
...
(function() {
  window.cheque = {
    init: function() {
      ...
      return app.nestedFormFieldRemoved('form', '.row-fluid', '.show-hide', '.fields:visible', '.amount');
    }
  };
...
(function() {
  window.credit_note = {
    init: function() {
      ...
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.tax_code'), '/tax_code/typeahead_code');
      });
      return credit_note.shared_init();
    },
    shared_init: function() {
      particular.init();
      matchers.init();
...
(function() {
  window.deposit = {
    init: function() {
      ...
      return ($('#cheque-query-btn')).click(function(e) {
        $.get('/depositable_cheques', {
          ...
          ($('#cheque-data .amount')).change();
          return ($('h4.no-record')).slideDown(1000);
        });
...
(function() {
  window.detail = {
    init: function() {
      ...
      ($('form')).on('change', '#details .package_qty', function() {
        var pack_data, qty_elm;
        ...
        return qty_elm.change();
      });
      app.typeahead_init('#details .packaging', '/packaging/typeahead_product_package_name', function(elm) {
        var product_json;
      ...
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        ...
        return app.typeahead_init(field.find('.packaging'), "/packaging/typeahead_product_package_name", function() {
          var product_json;
      ...
      ($('form')).on('change', '#details .tax_code', function() {
        var elm;
        elm = $(this);
        return $.get('/tax_code/json', {
          code: elm.val()
        }, function(data) {
          if (data) {
          ...
          return elm.parents('.fields').find('.gst_rate').change();
        });
      });
      ($('form')).on('change', '#details .product', function() {
        var elm;
        elm = $(this);
        return $.get('/product/json', {
          name1: elm.val(),
          gst_type: elm.parents('form').data('gst-type')
        }, function(data) {
          if (data) {
          ...
          return elm.parents('.fields').find('.tax_code').change();
        });
      });
      return ($('form')).on('change', '#details .packaging', function() {
        var elm, product_json;
        elm = $(this);
        product_json = elm.parents('.fields').find('.product').data('product-json');
        if (product_json) {
          $.get('/packaging/json', {
            ...
            if (!data) {
              ...
              return elm.parents('.fields').find('.package_qty').attr('disabled', false);
            } else {
              return elm.parents('.fields').find('.package_qty').removeAttr('disabled');
            }
          });
        }
        return elm.parents('.fields').find('.package_qty').change();
      });
    }
  };
...
(function() {
  window.invoice = {
    shared_init: function() {
      ...
      return app.showHide('#particulars .fields:visible', '#particulars .show-hide');
    },
    init: function() {
      invoice.shared_init();
      ...
      app.typeahead_init('.tax_code', '/invoice/typeahead_code');
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.tax_code'), '/invoice/typeahead_code');
      });
      return ($('.row-total')).change();
    }
  };
...
(function() {
  window.journal = {
    init: function() {
      ...
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.account'), '/account/typeahead_name1');
      });
...
(function() {
  $(function() {
    ...
    $(document).on('click', 'a:not([data-remote]):not([data-behavior]):not([data-skip-pjax]):not([data-method])', function(event) {
      var container;
      container = $('[data-pjax-container]');
      return $.pjax.click(event, container);
    });
    $(document).on('submit', 'form:not([data-skip-pjax])', function(event) {
      var container, val;
      container = $('[data-pjax-container]');
    ...
    ($('[data-pjax-container]')).on('pjax:end', function() {
      ...
      return app.standardInit();
    });
    return $('.alert, .no-record').fadeIn(1000);
  });
  ...
  window.math = {
    ...
    rowTotal: function(qtyCls, priceCls, discountCls, gstRateCls, gstCls, totalCls, rowCls, evtBubbleCls) {
      var calRowTotal;
      ($(evtBubbleCls)).on('change', qtyCls, function() {
        return calRowTotal(this);
      });
      ($(evtBubbleCls)).on('change', priceCls, function() {
        return calRowTotal(this);
      });
      ($(evtBubbleCls)).on('change', discountCls, function() {
        return calRowTotal(this);
      });
      ($(evtBubbleCls)).on('change', gstRateCls, function() {
        return calRowTotal(this);
      });
      return calRowTotal = function(elm) {
        var amount, discount, gst, gst_rate, gst_val, price, qty, row_total;
        ...
        gst.change();
        return row_total.change();
      };
    },
    sum: function(elements, totalElm, evtBubbleCls, checkVisible) {
      if (checkVisible == null) {
        checkVisible = true;
      }
      return ($(evtBubbleCls)).on('change', elements, function() {
        var total;
        total = 0;
        ($(elements)).each(function(index, elm) {
          var val;
          if (checkVisible) {
            val = ($(elm)).is(":visible") ? ($(elm)).val() : 0;
          } else {
            val = ($(elm)).val();
          }
  ...
  window.main = {
    init: function() {
      ($('#clear-advance-search-form')).click(function() {
        ...
        return ($('#advance-search-form .simple_form')).submit();
      });
      if (($('input#search_terms:last')).val() + ($('#search_date_from')).val() + ($('#search_date_to')).val() + ($('#search_amount_larger')).val() + ($('#search_amount_less')).val() !== "") {
        return ($('#advance-search-form')).collapse('show');
      }
    }
  };
  ...
  window.app = {
    ...
    avoidAutocompleteForTypeahead: function() {
      return ($('input.string[type="text"]')).attr('autocomplete', 'off');
    },
    initNumeric: function(selector) {
      if (selector == null) {
        selector = 'input.numeric:not([readonly])';
      }
      return ($(selector)).on('change', function() {
        ...
        if (!/[a-zA-Z]+/.test(($(this)).val())) {
          try {
            ...
            err = error;
            return ($(this)).parents('.control-group').addClass('error');
          }
        }
      });
    },
    initDatepicker: function() {
      return ($('input.datepicker:not([readonly])')).datepicker({
        dateFormat: 'dd-mm-yy',
    ...
    nestedFormFieldAdded: function(form, fields_parent, show_hide_elm, func) {
      return ($(form)).on("nested:fieldAdded", function(event) {
        if (func) {
          func(event.field);
        }
    ...
    nestedFormFieldRemoved: function(form, fields_parent, show_hide_elm, count_fields, trigger_blur_elements) {
      return ($(form)).on("nested:fieldRemoved", function(event) {
        var row_parent;
        row_parent = event.field.closest(fields_parent);
        ($(trigger_blur_elements)).change();
        if (row_parent.find(count_fields).size() === 0) {
          return row_parent.find(show_hide_elm).hide();
        }
      });
    },
    showHide: function(field_selector, showhide_selector) {
      if (($(field_selector)).size() > 0) {
        return ($(showhide_selector)).show();
      } else {
        return ($(showhide_selector)).hide();
      }
    },
    standard_row_total_init: function() {
      return math.rowTotal('.quantity', '.unit-price', '.discount', '.gst_rate', '.gst', '.row-total', '.fields', '.row-fluid');
    },
    typeahead_init: function(selector, url, params_func) {
      var elms;
      if (params_func == null) {
        params_func = function() {
          return {};
        };
      }
      if (selector.jquery) {
      ...
      return ($(elms)).each(function(index, elm) {
        return ($(elm)).typeahead({
          source: function(q, p) {
            return $.get(url, $.extend({
              term: q
            }, params_func($(elm))), function(data) {
              return p(data);
            });
          },
          matcher: function() {
...
(function() {
  window.matchers = {
    init: function() {
      ($('#matcher-query-btn')).click(function(e) {
        $.get('/matchers', {
          ...
          ($('.matching')).change();
          return app.initNumeric('.matching.numeric');
        });
        return e.preventDefault();
      });
      ($('form')).on('click', '.auto_match', function() {
        var balance;
        balance = +($(this)).closest('.fields').find('.balance').val();
        ($(this)).closest('.fields').find('.matching').val(-balance);
        return ($('.matching')).change();
      });
      return ($('form')).on('change', '.matching', function() {
        var amount, balance, matched, matching, self_matched;
        ...
        if (matching === 0) {
          return ($(this)).closest('.fields').find('[type=hidden][name*=_destroy]').val(true);
        } else {
          return ($(this)).closest('.fields').find('[type=hidden][name*=_destroy]').val(false);
        }
      });
    }
  };
...
(function() {
  window.packaging = {
    init: function() {
      ($('form')).on('change ', '#products .product', function() {
        var elm;
        elm = $(this);
        return $.get('/product/json', {
          name1: elm.val()
        }, function(data) {
          return elm.parents('.fields').find('[name="row_unit"]').val(data.unit);
        });
      });
      app.typeahead_init('.product', '/product/typeahead_name1');
      return app.nestedFormFieldAdded('form', '.row-fluid.nested', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.product'), '/product/typeahead_name1');
      });
    }
  };
...
(function() {
  window.particular_type = {
    init: function() {
      return app.typeahead_init('#particular_type_tax_code_code', '/tax_code/typeahead_code');
    }
  };
...
(function() {
  window.particular = {
    init: function() {
      ...
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.particular_type'), '/particular_type/typeahead_name');
      });
      ($('form')).on('change', '#particulars .particular_type', function() {
        var elm;
        elm = $(this);
        return $.get('/particular_type/json', {
          name: elm.val(),
          gst_type: elm.parents('form').data('gst-type')
        }, function(data) {
          elm.parents('.fields').find('.tax_code').val(data.tax_code);
      ...
      ($('form')).on('change', '#particulars .tax_code, #pay-to-particulars .tax_code, #pay-from-particulars .tax_code', function() {
        var elm;
        elm = $(this);
        return $.get('/tax_code/json', {
          code: elm.val()
      ...
      return app.nestedFormFieldRemoved('form', '.row-fluid', '.show-hide', '.fields:visible', '.row-total');
    }
  };
...
(function() {
  window.pay_slip = {
    init: function() {
      ...
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.salary_type'), '/salary_type/typeahead_name');
      });
      app.nestedFormFieldRemoved('form', '.row-fluid', '.show-hide', '.fields:visible', '.row-total');
      ($('form')).on('blur', '#pay_slip_pay_from_name1', function() {
        if (($(this)).val() === 'Cash In Hand') {
          ($('#pay_slip_chq_no')).parents('.control-group').hide();
          return ($('#pay_slip_chq_no')).val('');
        } else {
          ($('#pay_slip_chq_no')).parents('.control-group').show();
          return ($('#pay_slip_chq_no')).select();
        }
      });
      ($('form')).on('change', '#notes .salary_type', function() {
        var elm;
        elm = $(this);
        return $.get('/salary_type/json', {
          name: elm.val()
        }, function(data) {
          var row;
          ...
          return elm.parents('.fields').find('.quantity').change();
        });
      });
      ($('form')).on('change', '.row-total', function() {
        var addition, advance, deduction;
        ...
        $('#total_notes').val(math.round(addition - deduction, 2));
        return $('#total_pay').val(math.round(addition - deduction - advance, 2));
      });
      return $('.quantity').change();
    },
    sum_element: function(selector) {
      var total;
      total = 0;
...
(function() {
  window.payment = {
    shared_init: function() {
      ...
      return math.sum('#total_pay_to_gst, #payment_actual_debit_amount, #total_pay_from_particulars', '#payment_actual_credit_amount', 'form#payment');
    },
    init: function() {
      payment.shared_init();
      app.typeahead_init('.tax_code', '/tax_code/typeahead_code');
...
(function() {
  window.product = {
    init: function() {
      ...
      return app.nestedFormFieldAdded('form', '.row-fluid.nested', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.packaging'), '/packaging/typeahead_name');
      });
    }
  };
...
(function() {
  window.pur_invoice = {
    init: function() {
      invoice.shared_init();
      app.typeahead_init('.tax_code', '/pur_invoice/typeahead_code');
      app.nestedFormFieldAdded('form', '.row-fluid', '.show-hide', function(field) {
        return app.typeahead_init(field.find('.tax_code'), '/pur_invoice/typeahead_code');
      });
...
(function() {
  window.product_sales_report = {
    init: function() {
      ...
      return ($('#options_product_tags')).select2({
  ...
  window.sales_amount_report = {
    init: function() {
      return ($('#options_doc_tags')).select2({
        tags: $('#options_doc_tags').data('tags'),
        closeOnSelect: true,
        openOnEnter: false,
        minimumInputLength: 2
...
(function() {
  window.return_cheque = {
    init: function() {
      ...
      ($('form')).on('change', '#return_cheque_return_to_name1', function() {
        ...
        return ($('#return_cheque_amount')).val("");
      });
      app.typeahead_init('#return_cheque_reason', '/return_cheque/typeahead_reason');
      return ($('#rtn-cheque-query-btn')).click(function(e) {
        $.get('/get_return_cheque', {
          ...
          if (data) {
            ...
            return ($('#return_cheque_amount')).val(data.amount);
          } else {
            ($('#return_cheque_cheque_id')).val(null);
            ...
            return ($('#return_cheque_amount')).val('Not Found!');
          }
        });
        return e.preventDefault();
      });
...
(function() {
  window.salary_note = {
    init: function() {
      ...
      return ($('form')).on('change', '#salary_note_quantity, #salary_note_unit_price', function() {
        var price, qty;
        qty = +($('#salary_note_quantity')).val();
        price = +($('#salary_note_unit_price')).val();
        return ($('#salary_note_amount')).val(math.round(qty * price, 2));
      });
    }
  };
...
(function() {
  window.transaction = {
    init: function() {
      ...
      return app.typeahead_init('#transactions_query_name', '/account/typeahead_name1');
    }
  };
